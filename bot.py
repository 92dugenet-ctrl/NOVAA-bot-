...
import threading
import gc  # ← Ajouté pour libérer la mémoire
...
# LIMITES RÉDUITES pour économiser la mémoire
MAX_ENTREPRISES_PAR_SESSION = 15  # Réduit de 30 à 15
MAX_EMAILS_PAR_SESSION = 10       # Réduit de 20 à 10

# ============================================
# TÉLÉCHARGEMENT PLAQUETTE (optimisé)
# ============================================
...
def scraper_entreprises(activite, ville):
    ...
    try:
        ...
        # Libérer la mémoire de la réponse
        response.close()
        ...
        for carte in soup.find_all(["div", "article"], class_=re.compile(r"carte|card|fiche|result")):
            ...
            if nom and not site:
                ...
                if len(nom_texte) > 2:
                    ...
                    # Limiter la collecte pour économiser la mémoire
...
async def prospect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for e in entreprises[:MAX_EMAILS_PAR_SESSION]:
        if envoyer_email(e["email"], e["nom"]):
            envoyes += 1
        time.sleep(2)
        gc.collect()  # Nettoyer après chaque envoi
